      ...
      <div className="flex-1 flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-20 px-6 md:px-12 lg:px-20 py-12 lg:py-0 max-w-7xl mx-auto w-full">
        ...
        <div className="flex-1 max-w-xl space-y-8">
          ...
          <div className="flex gap-8 pt-4">
            {[
              { icon: BookOpen, label: 'Books tracked', value: '∞' },
              { icon: BarChart3, label: 'Templates', value: '4' },
              { icon: Download, label: 'Export', value: 'HD' },
            ].map(({ icon: Icon, label, value }) => (
              <div key={label} className="flex items-center gap-2">
                <Icon className="w-4 h-4 text-hero-btn" />
                <div>
                  <p className="text-hero-foreground font-display font-bold text-sm">{value}</p>
                  <p className="text-hero-sub text-[10px] font-body">{label}</p>
                </div>
              </div>
            ))}
          </div>
